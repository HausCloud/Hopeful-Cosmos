$(function() {
  ...
  function geo_success(position) {
    ...
    // Request Google Reverse Geocode API and parse location JSON
  ...
  // Callback to display state and country field on error
  function geo_failure(err) {
    console.log('Hopeful Cosmos couldn\'t retrieve your location. ERROR: ' + err)
    $('#statefieldcontainer').show(800)
    $('#countryfieldcontainer').show(800)
  }

  // Check browser for geolocation support otherwise display respective fields
  if ("geolocation" in navigator) {
    navigator.geolocation.getCurrentPosition(geo_success, geo_failure, options);
  } else {
    geo_failure('Geolocation not supported');
  }
  ...
  $.ajax({
    ...
    success: function(data) {
      ...
      for (const key in data) {
        // Create div to be added to universe container
	...
	// Apple styling to each star
        $(`.starcontainer>[wish_id=${key}]`).css({
          "background": "white",
          "height": `${num}`,
          "width": `${num}`,
          "border-radius": "50%",
  ...
  // Form validation plugin with giant regex
  ...
  $("#wishform").validate({
    ...
    submitHandler: function(form) {
      ...
      // Prepare wish object for POST request
